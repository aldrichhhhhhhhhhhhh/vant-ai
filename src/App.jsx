...
function ChatPage({ theme, isDark, onGoToIntegrations }) {
  ...
  async function send(text) {
    ...
    setMessages(next);
    setStarted(true);
    if (inputRef.current) { inputRef.current.value = ""; inputRef.current.style.height = "42px"; }
    setLoading(true);
    setComposerNotice("");
  ...
  function handleSubmit(e) { e.preventDefault(); const text = inputRef.current?.value?.trim() || ""; if (!text || loading) return; send(text); }
    ...
    return (
      <div style={{ width: "100%", maxWidth: compact ? 920 : 720, margin: compact ? "0 auto" : "36px auto 0" }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: "flex", alignItems: "flex-end", gap: 8, padding: 7, borderRadius: 18, background: theme.inputBg, border: `1px solid ${theme.borderStrong}`, boxShadow: isDark ? "0 8px 30px rgba(0,0,0,.16)" : "0 8px 30px rgba(15,15,35,.06)" }}>
          <ComposerMenu />
          <textarea ref={inputRef} onInput={(e) => { e.currentTarget.style.height = "42px"; e.currentTarget.style.height = `${Math.min(e.currentTarget.scrollHeight, 130)}px`; }} onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); handleSubmit(e); } }} placeholder="Ask VANT to do something…" disabled={loading} rows={1} style={{ ...inputStyle, flex: 1, minHeight: 42, maxHeight: 130, resize: "none", border: "none", background: "transparent", padding: "11px 8px", borderRadius: 12, boxSizing: "border-box" }} />
          <button type="submit" disabled={loading} title="Send" style={{ width: 42, height: 42, borderRadius: 12, background: acBg("violet"), border: "none", display: "flex", alignItems: "center", justifyContent: "center", cursor: loading ? "default" : "pointer", opacity: loading ? 0.45 : 1 }}><Send size={17} color={ac("violet", isDark)} /></button>
        </form>
